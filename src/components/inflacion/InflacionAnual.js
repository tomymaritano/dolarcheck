...
const InflationAnual = () => {
    ...
    useEffect(() => {
        const fetchInflationData = async () => {
            try {
                ...
                const years = new Set(data.map(item => new Date(item.fecha).getFullYear().toString()));
    ...
    const filterDataByYear = (data, year) => {
        const filteredData = data.filter(item => new Date(item.fecha).getFullYear().toString() === year);
