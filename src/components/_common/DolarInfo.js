    ...
    const fetchDolarData = async () => {
        ...
        try {
            ...
            if (allData.length > 0) {
                ...
                const filteredDataToday = allData.filter(data => data.fecha === lastDate);
                const filteredDataYesterday = allData.filter(data => data.fecha === prevDate.toISOString().split('T')[0]);
...
export default DolarCotizaciones;
